...
use \yottahead_\Dtdr_Controller as Contr;
...
class Dtdr_Load{
    ...
    public $option_keys = 'themeim-dark-options-settings';
    ...
    public function init_menu(){
        ...
        do_action('yotta_header_admin_menu_start');
        
        add_menu_page(
			__( 'Create Template', 'yottahead_' ),
			__( 'Themeim Builder', 'yottahead_' ),
			'edit_pages',
            'edit.php?post_type=themeim-builder',
            '',
            'dashicons-chart-pie',
            7
        );

        do_action('yotta_header_admin_menu_end');
    }
    ...
    public function admin_enqueue(){
        ...
        if( in_array($screen->id, [ 'toplevel_page_droit-head-settings', 'themeim-builder', 'edit-themeim-builder']) ){
        ...
        if( ( 'post.php' == $pagenow || 'post-new.php' == $pagenow ) || in_array($screen->id, [ 'toplevel_page_droit-head-settings', 'themeim-builder', 'edit-themeim-builder']) ){
