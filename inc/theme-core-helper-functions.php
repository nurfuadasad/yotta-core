...
if (!class_exists('Yotta_Core_Helper_Functions')) {

    class Yotta_Core_Helper_Functions
    {
        /**
         * $instance
         * @since 1.0.0
        ...
        function post_thumbnail()
        {
            ...
            if (is_singular()) :
                ?>
                <?php the_post_thumbnail('yotta_classic'); ?>
            <?php else : ?>
                <a href="<?php the_permalink(); ?>" aria-hidden="true" tabindex="-1">
                    <?php
                    the_post_thumbnail('yotta_classic', array(
        ...
        public function posted_tag()
        {
            ...
            if ($tags_list) {
                ...
                printf('<ul class="tags"><li class="title">' . esc_html__('Tags: ', 'yotta') . '</li><li>' . ' %1$s' . '</li></ul>', $tags_list); // WPCS: XSS OK.
            }
        }
        ...
        public function post_navigation()
        {
            the_post_navigation(array(
                'prev_text' => '<i class="fas fa-angle-double-left"></i>&nbsp;' . esc_html__('Prev Post', 'yotta'),
                'next_text' => esc_html__('Next Post', 'yotta') . '&nbsp;<i class="fas fa-angle-double-right"></i>',
            ));
            echo wp_kses('<div class="clearfix"></div>', yotta()->kses_allowed_html('all'));
        }
        ...
        public function get_contact_form_shortcode_list_el()
        {
            ...
            if ($forms) {
                ...
                $forms_list[esc_html__('No contact form found', 'yotta-core')] = 0;
            }
            return $forms_list;
        }
        ...
        {
            $course_id = get_the_ID();
            $enroll_btn = '<div  class="tutor-loop-cart-btn-wrap">' . apply_filters('tutor_course_restrict_new_entry', '<a href="' . get_the_permalink() . '">' . esc_html__('Get Enrolled', 'yotta-core') . '</a>') . '</div>';
        ...
        {
            return defined('YOTTA_CORE_SELF_PATH') ? true : false;
        }
        ...
        public function getPostViews($postID)
        {
            ...
            if ($count == '') {
                ...
                printf(' 0 %s', esc_html__('Views', 'yotta'));
            }
            printf($count . ' %s ', esc_html__('Views', 'yotta'));
        }
        ...
        public function render_elementor_icons($settings, $attr = [])
        {
            ...
            return \Yotta\Yotta_elementor_icon_manager::render_icon($settings, $attr);
        }
    } //end class

    if (class_exists('Yotta_Core_Helper_Functions')) {
        Yotta_Core_Helper_Functions::getInstance();
    }
}
